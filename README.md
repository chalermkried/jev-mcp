...
The upstream origin is fixed and redirects are refused. Jev requests have a 15-second timeout and no automatic retries to avoid multiplying billable work. A 25-second request budget covers body reading and upstream calls; the Vercel MCP function has a 30-second duration limit. The MCP function [opts into Vercel request cancellation](https://vercel.com/docs/functions/functions-api-reference#enable-cancellation), so client disconnects can abort an in-flight Jev call through `request.signal`. The concurrency ceiling is local to a warm process and does not limit aggregate traffic across instances. Vercel WAF/IP rules can add edge protection later; there is no database or distributed quota service.
