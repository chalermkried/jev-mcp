...
- Docs: README restructured around the eight tools (capability bullets, collapsible per-client install sections), the `jev_rerank` example replaced with a live-captured semantic-ranking example that shows where candidate text comes from, and guidance added for ranking whole documents by chunking with distinct ids.
